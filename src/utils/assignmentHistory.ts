...
export class AssignmentHistoryManager {
  ...
  static saveAssignmentToHistory(protectionAssignmentData: ProtectionAssignmentData, assignmentId: string, protectionOfficerName?: string): void {
    try {
      ...
      console.error('Error saving assignment to history:', error);
    }
  }
   ...
   */
  static getAssignmentHistory(): ProtectionAssignmentHistoryItem[] {
    try {
      const stored = localStorage.getItem(STORAGE_KEYS.ASSIGNMENT_HISTORY);
      return stored ? JSON.parse(stored) : [];
    } catch (error) {
      console.error('Error retrieving assignment history:', error);
      return [];
    }
  }
  ...
  private static updatePersonalizationAnalytics(historyItem: ProtectionAssignmentHistoryItem): void {
    try {
      ...
      const serviceCount = this.getAssignmentHistory()
        .reduce((acc, item) => {
