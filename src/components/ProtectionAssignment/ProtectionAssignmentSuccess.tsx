...
import styles from './ProtectionAssignmentSuccess.module.css';

interface ProtectionAssignmentSuccessProps {
  assignmentId: string;
}

export function ProtectionAssignmentSuccess({ assignmentId }: ProtectionAssignmentSuccessProps) {
  const { navigateToView } = useApp();

  useEffect(() => {
    // Save assignment to history on mount
    try {
      const preserved = localStorage.getItem('armora_booking_state'); // Keep legacy storage key
      if (preserved) {
        ...
        if (protectionAssignmentData) {
          ...
          const officerNames = ['John S.', 'Sarah M.', 'David L.', 'Emma R.', 'Michael T.', 'Lisa K.'];
          const randomOfficer = officerNames[Math.floor(Math.random() * officerNames.length)];

          AssignmentHistoryManager.saveAssignmentToHistory(protectionAssignmentData, assignmentId, randomOfficer);
          console.log('✅ Protection assignment saved to history:', assignmentId);
        }
      }
    } catch (error) {
      console.error('❌ Failed to save assignment to history:', error);
    }
